...
def train():
    ...
    while True:
        ...
        # h5SavePath=curfile+'\myModel'
        '''
        To save the model during training, we have to know the shapes of models' inputs.
        And set the shape in the model definition, using self._set_inputs()
        '''
        '''
        seq2seqModel.encoder.save("myEncoder")
        seq2seqModel.decoder.save("myDecoder")
        seq2seqModel.optimizer.save("myOptimizer")
        print("已保存 Encoder/Decoder/Optimizer")
        seq2seqModel.checkpoint.save(file_prefix=checkpoint_prefix)
        print("保存checkpoint")
        '''
        # some changes to save a h5 model
